...
### Changed
- 19개 파일의 구식 스킬 호출(`/project-init` 등) → `/adt:project-init` 등으로 갱신
- 사용자 대면 docs, 템플릿, 스킬 간 교차 참조, rules, 훅 경고 메시지, 참조 문서 전체 일관성 확보

### Note
- 플러그인 content 변경 시 버전 증가 필수 — `claude plugin update`는 version 변경이 있을 때만 실제로 재다운로드
