...
[유지보수 — 필요 시]
       │
       ├─→ /adt:project-absorb   (프로젝트 → 라이브러리)
       ├─→ /adt:project-update    (라이브러리 → 프로젝트)
       └─→ /adt:optimize-docs     (지침 파일 줄 수 관리)
```

---

## 1. 워크스테이션 세팅 — `/adt:ai-platform-defconfig`
...
3. 템플릿 파일 복제:
   ...
   - `.claude/skills/adt:project-configure/SKILL.md`
...
└── .claude/skills/adt:project-configure/
    └── SKILL.md                           ← 구체화 스킬
```

> **참고**: init만으로는 placeholder(`{{기술_스택}}` 등)가 남아있다.
> 반드시 `/adt:project-configure`로 구체화해야 한다.

---

## 3. 프로젝트 설정 구체화 — `/adt:project-configure`

**언제**: `/adt:project-init` 직후, 또는 프로젝트 설정을 재구성할 때
...
## 4. 프로젝트 업데이트 — `/adt:project-update`

**언제**: 라이브러리에 새 버전이 릴리스된 후, 기존 프로젝트에 적용할 때

**실행**:
```
/adt:project-update ~/my-project
```
...
## 5. 지침 흡수 — `/adt:project-absorb`

**언제**: 프로젝트에서 개선한 지침이 다른 프로젝트에도 유용할 때

**실행**:
```
/adt:project-absorb ~/my-project
```
...
blueprints/coding-standards.md에 해당 규칙 반영
       ↓ 라이브러리 Minor 버전 증가
       ↓ /adt:project-update (다른 프로젝트에 전파)
```

---

## 6. 문서 최적화 — `/adt:optimize-docs`
...
`/adt:project-init`이 기존 파일을 감지하여 신규 추가/갱신을 판단한다.
기존 커스텀 내용은 보존된다.

### Q: 여러 워크스테이션에서 동시에 사용할 수 있나?

가능하다. 각 워크스테이션에서 클론 후 `/adt:ai-platform-defconfig`를 실행하면 된다.
...
현재는 프로젝트별로 `/adt:project-update`를 개별 실행한다.
